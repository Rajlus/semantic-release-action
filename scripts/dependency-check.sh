...
for (const pkg of corePackages) {
  ...
  if (!info) {
    // Not in npm outdated output — either up to date or not installed
    if (allDeps[pkg]) {
...
if (currentResults.length > 0) {
  md += '<details><summary>✅ Up to date (' + currentResults.length + ' packages)</summary>\n\n';
...
if (otherResults.length > 0) {
  for (const r of otherResults) {
    md += '> ⚠️ **' + r.pkg + '**: ' + (r.status === 'not-installed' ? 'Not found in package.json' : 'Could not determine version') + '\n';
  }
  md += '\n';
}
...
if (warningCount > 0) {
  ...
  md += '> All **' + corePackages.length + '** core packages are on the latest major version. ✅\n\n';
}
